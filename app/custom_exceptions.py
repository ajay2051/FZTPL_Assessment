...
class NoGPSData(BaseError):
    """
    Error raised when no GPS data is available
    """
    pass
...
class BookingNotFound(BaseError):
    """
    Error raised when no Booking Data is available
    """
    pass


class GPSDeviceNotFound(BaseError):
    """
    Error raised when no Booking Data is available
    """
    pass
...
class BlogsNotFound(BaseError):
    """
    Error raised when no Blogs are found
    """
    pass
...
def register_all_errors(app: FastAPI):
    ...
    app.add_exception_handler(NoGPSData, create_exception_handler(
        status_code=status.HTTP_404_NOT_FOUND,
        message={
            "message": "No GPS Data <UNK>...⛔👿️️️",
            "error-code": "no-gps-data",
        }
    ))

    app.add_exception_handler(BookingNotFound, create_exception_handler(
        status_code=status.HTTP_404_NOT_FOUND,
        message={
            "message": "Booking Not Found...⛔👿️️️",
            "error-code": "no-booking-data",
        }
    ))

    app.add_exception_handler(GPSDeviceNotFound, create_exception_handler(
        status_code=status.HTTP_404_NOT_FOUND,
        message={
            "message": "GPS Device Not Found...⛔👿️️️",
            "error-code": "no-gps-device",
        }
    ))

    app.add_exception_handler(BlogsNotFound, create_exception_handler(
        status_code=status.HTTP_404_NOT_FOUND,
        message={
            "message": "Blogs Not Found...⛔👿️️️",
            "error-code": "no-blogs-data",
        }
    ))
